...
describe('Menu', () => {
    ...
    it('should call toggleCurrentSelected when a menu item is clicked', () => {
        //Setup
        //Execution
        const mockToggleCurrentSelected = jest.fn();
        const wrapper = shallow(<Menu menu={mockAppState} toggleCurrentSelected={mockToggleCurrentSelected}/>)
        
        // wrapper.props.toggleCurrentSelected = jest.fn();
        wrapper.find('.local').simulate('click', mockEvent1);
        //Expectation
        expect(mockToggleCurrentSelected).toHaveBeenCalledWith(mockEvent1)
    })
})